...
class ClassLaunch:
    def __init__(
        ...
        launch_outcome: str,
        location,
        booster_landing_outcomes: list[str],
        landing_zones,
        secondstage,
        secondstage_flights: int,
        booster_lat: float,
        booster_long: float,
        booster_support,
        booster_tug: str,
        fairing_flights: list[str],
        fairings_recovered: list[str],
        fairing_recovery_boats,
        fairing_lat: float,
        fairing_long: float,
...
class LaunchCalculator:
    ...
    def __eq__(
        self, object
    ):  # This function allows for checking if the booster/pad already exists; if the name is the same later in the code we ignore it
...
class GetSheets(LaunchCalculator):
    ...
    def __get_or_add_object__(
        self,
        name: str,
        search_list: list[LaunchCalculator],
        object: LaunchCalculator,
        **kwargs,
    ) -> LaunchCalculator:
    ...
    def __parse_falcon_9_row__(self, row: list[str], launch_boosters: list[Booster]):
        ...
        booster = self.__get_or_add_object__(
            booster_name,
            self.boosters,
            Booster,
            rocket="Falcon 9",
        ...
        for index, booster_name in enumerate(booster_name.split(",")):
            ...
            if index == 0:
                booster = self.__get_or_add_object__(
                    ...
                    Booster,
                    rocket="Falcon Heavy",
                    version=booster_version.replace("FH ", ""),
                )
            else:
                booster = self.__get_or_add_object__(
                    booster_name,
                    ...
                    Booster,
                    rocket="Falcon 9",
                    version=booster_version.replace("F9 ", "").lstrip(" "),
                )
    ...
    def __parse_starship_row__(self, row: list[str], launch_boosters: list[Booster]) -> SecondStage:
        ...
        return self.__get_or_add_object__(
            secondstage_name,
            self.secondstages,
            SecondStage,
            rocket="Starship",
        ...
        for row in self.reader:
            ...
            if not row[20] == "N/A":  # Creates the fairing recovery ships
                for ship in row[20].split("/"):
                    fairing_recovery_ships.append(
                        self.__get_or_add_object__(
                            ship.lstrip(" ").rstrip(" "),
                            self.fairing_recovery_ships,
                            ClassFairingRecovery,
                        )
                    )

            if not row[16] == "N/A":  # Creates the support ships
                for ship in row[16].split("/"):
                    support_ships.append(
                        self.__get_or_add_object__(
                            ship.lstrip(" ").rstrip(" "),
                            self.support_ships,
                            SupportShip,
            ...
            if second_stage_landing_zone is not None:
                self.__get_or_add_object__(
                    second_stage_landing_zone.name, self.landing_zones, ClassLandingZone
...
for booster in falcon_data.boosters:
    ...
    Stage.objects.get_or_create(
        name=booster.name,
        rocket=Rocket.objects.get_or_create(name=str(booster.rocket))[0],
        version=booster.version,
        type="BOOSTER",
    )
...
for second_stage in falcon_data.secondstages:
    ...
    Stage.objects.get_or_create(
        ...
        rocket=Rocket.objects.get_or_create(name=str(second_stage.rocket))[0],
...
for launch in falcon_data.launches:  # noqa: MC001
    ...
    Launch.objects.get_or_create(
        time=launch.time,
        pad=Pad.objects.get(nickname=launch.location),
        rocket=Rocket.objects.get(name=launch.rocket),
        name=launch.payload,
        orbit=Orbit.objects.get(name=launch.orbit),
        mass=launch.mass,
    ...
    for index, booster in enumerate(launch.boosters):
        ...
        if (
            ...
            and launch.landing_zones[index].name is not None
            and (
                launch.landing_zones[index].name == "LZ-1"
                or launch.landing_zones[index].name == "LZ-2"
                or launch.landing_zones[index].name == "LZ-4"
            )
        ):
            method = "GROUND_PAD"
        if (
            ...
            and launch.landing_zones[index].name is not None
            and (
                launch.landing_zones[index].name == "OCISLY"
                or launch.landing_zones[index].name == "JRtI"
                or launch.landing_zones[index].name == "ASoG"
            )
        ):
            method = "DRONE_SHIP"
        ...
        if (
            "Failure" in launch.booster_landing_outcomes[index]
    ...
    for index, fairing_recovery in enumerate(launch.fairings_recovered):
        ...
        if (
            ...
            and launch.fairings_recovered[index] is not None
            and "Catch" in launch.fairings_recovered[index]
        ):
            catch = True
        ...
        if not len(launch.fairing_recovery_boats) == 0:
            with transaction.atomic():
                ...
                boat_instance, _ = Boat.objects.get_or_create(
                    name=launch.fairing_recovery_boats[index], type="FAIRING_RECOVERY"
                )
