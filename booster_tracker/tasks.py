...
from booster_tracker.models import StageAndRecovery, Launch, Stage
...
@shared_task
def update_cached_stageandrecovery_value_task(stage_id, zone_id):

    # Update all related StageAndRecovery instances
    related_stage_and_recoveries = StageAndRecovery.objects.filter(stage__id=stage_id, landing_zone__id=zone_id)
    ...
    related_launch_instances = Launch.objects.filter(stageandrecovery__stage__id=stage_id)
...
def update_cached_launch_value_task():
    ...
    for launch in related_launches:
        ...
        launch.save(update_fields=["company_turnaround", "pad_turnaround", "image"])
        launch._from_task = False
